...
DEEPGRAM_AUTH_KEY = os.environ['DEEPGRAM_AUTH_KEY']
DEEPGRAM_AUTH_SECRET = os.environ['DEEPGRAM_AUTH_SECRET']
...
def start_call() -> dict:
    ...
    call_url = "https://api.twilio.com/2010-04-01/Accounts/" + TWILIO_ACCOUNT_SID + "/Calls/" + sid + ".json"
...
def transcribe() -> dict:
    ...
    deepgram_req = requests.post(
        ...
        auth = (DEEPGRAM_AUTH_KEY, DEEPGRAM_AUTH_SECRET),
        json = { "url": body["audio_url"]}
    )
    print('done processing request, sending deepgram response back to client', deepgram_req.text)
